	...
	$.fn.extend({
		mySlider: function(options) {
			...
			appendIco = function() {
				...
				iset.imgIco.find('span').eq(0).addClass(iset.active);
			};
			/*点击事件*/
			var icoClick = function(){
				iset.imgIco.find('span').click(function(){
			...
			var autoScroll = function() {
				if (iset.direction) {
					i++;
					if (i >= imgLi.length) {
						iset.imgIco.find('span').eq(0).addClass(iset.active).siblings().removeClass(iset.active);
					};
					if (i > imgLi.length) {
						imgUl.css({
							left: 0
					...
					iset.imgIco.find('span').eq(i).addClass(iset.active).siblings().removeClass(iset.active);
				} else {
					i--;
					if (i == -1) {
						imgUl.css({
